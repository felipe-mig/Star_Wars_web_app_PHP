...
$id = $_GET['id'];
$title = returnMovieDetails($dbConnect, $id, 'title');
$intro = returnMovieDetails($dbConnect, $id, 'intro');
$sypnosis = returnMovieDetails($dbConnect, $id, 'sypnosis');
$posterimage = returnMovieDetails($dbConnect, $id, 'posterimage');
$background = returnMovieDetails($dbConnect, $id, 'background');
$imageone = returnMovieDetails($dbConnect, $id, 'imageone');
$imagetwo = returnMovieDetails($dbConnect, $id, 'imagetwo');
...
<body>
    ...
    <div class="wrapper"> 
        ...
        <div class="mainRightContainer">
            ...
            <form class="addCharacterForm" action="movieUpdateAction.php" method="post">
                ...
                <input class="inputRegular" type="text" name="title" required value="<?php echo $title ?>"/>
                <br>

                <label class="regularLabel">introduction phrase</label>
                <input class="inputRegular" type="text" name="intro" required value="<?php echo $intro ?>">
                <br>

                <label class="regularLabel">sypnosis:</label>
                <textarea type="text" name="sypnosis" required><?php echo $sypnosis ?></textarea>
                <br>
                <br>
                ...
                <select name="posterimage" id="files">
                    <option value="" disabled selected hidden><?php echo $posterimage ?></option>
                    <?php
                ...
                <select name="background" id="files">
                    <option value="" disabled selected hidden><?php echo $background ?></option>
                    <?php
                        foreach ($files as $file) {
                            // Skip current and parent directory 
                            if ($file !== '.' && $file !== '..') {
                                $selected = ""; 
                                if ($file === $background) { $selected = "selected"; 
                                }  echo "<option value=\"" . $file . "\" " . $selected . ">" . $file . "</option>";
                            }
                        }
                    ?>
